...
const PostDetailPage = ({ id }: { id: string }) => {
    ...
    useEffect(() => {
        id ? fetchPost(id) : console.error('Provide Valid Url')
    }, [fetchPost, id])
    ...
    return (
        <div className='grid gap-5 col-span-8'>
            ...
            {selectedPost.content && <div dangerouslySetInnerHTML={{ __html: DOMPurify.sanitize(selectedPost.content)}} className="grid gap-4"></div>}
        </div>
    );
}
